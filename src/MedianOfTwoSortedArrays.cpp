...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        return (findkth(nums1, 0, m, nums2, 0, n, (m + n + 1) / 2)
                + findkth(nums1, 0, m, nums2, 0, n, (m + n + 2) / 2)) / 2.0;
    }
private:
    double findkth(vector<int>& nums1, int l1, int m,
                    vector<int>& nums2, int l2, int n, int k)
    {
        if(m > n)   return findkth(nums2, l2, n, nums1, l1, m, k);
